...
const SwapfishFactory = artifacts.require("./SwapfishFactory.sol");
const SwapfishPair = artifacts.require("./SwapfishPair.sol");
const SwapfishRouter = artifacts.require("./swapfishRouter.sol");
const swapfishZapV1 = artifacts.require("./swapfishZapV1.sol");
const WBNB = artifacts.require("./WBNB.sol");

contract("swapfishZapV1", ([alice, bob, carol, david, erin]) => {
  ...
  before(async () => {
    // Deploy Factory
    SwapfishFactory = await SwapfishFactory.new(alice, { from: alice });

    // Deploy Wrapped BNB
    wrappedBNB = await WBNB.new({ from: alice });

    // Deploy Router
    SwapfishRouter = await swapfishRouter.new(SwapfishFactory.address, wrappedBNB.address, { from: alice });

    // Deploy ZapV1
    maxZapReverseRatio = 100; // 1%
    swapfishZap = await swapfishZapV1.new(wrappedBNB.address, swapfishRouter.address, maxZapReverseRatio, { from: alice });
    ...
    // Create 3 LP tokens
    let result = await SwapfishFactory.createPair(tokenA.address, wrappedBNB.address, { from: alice });
    pairAB = await SwapfishPair.at(result.logs[0].args[2]);

    result = await SwapfishFactory.createPair(wrappedBNB.address, tokenC.address, { from: alice });
    pairBC = await SwapfishPair.at(result.logs[0].args[2]);

    result = await SwapfishFactory.createPair(tokenA.address, tokenC.address, { from: alice });
    pairAC = await SwapfishPair.at(result.logs[0].args[2]);
    ...
    for (let thisUser of [alice, bob, carol, david, erin]) {
      ...
      await tokenA.approve(swapfishRouter.address, constants.MAX_UINT256, {
        from: thisUser,
      });

      await tokenA.approve(swapfishZap.address, constants.MAX_UINT256, {
        from: thisUser,
      });

      await tokenC.approve(swapfishRouter.address, constants.MAX_UINT256, {
        from: thisUser,
      });

      await tokenC.approve(swapfishZap.address, constants.MAX_UINT256, {
        from: thisUser,
      });

      await wrappedBNB.approve(swapfishRouter.address, constants.MAX_UINT256, {
        from: thisUser,
      });

      await wrappedBNB.approve(swapfishZap.address, constants.MAX_UINT256, {
        from: thisUser,
      });

      await pairAB.approve(swapfishZap.address, constants.MAX_UINT256, {
        from: thisUser,
      });

      await pairBC.approve(swapfishZap.address, constants.MAX_UINT256, {
        from: thisUser,
      });

      await pairAC.approve(swapfishZap.address, constants.MAX_UINT256, {
        from: thisUser,
      });
    }
  });
  ...
  describe("Normal cases for liquidity provision and zap ins", async () => {
    it("User adds liquidity to LP tokens", async function () {
      ...
      // 1 A = 1 C
      let result = await swapfishRouter.addLiquidity(
        tokenC.address,
        tokenA.address,
      ...
      // 1 BNB = 100 A
      result = await swapfishRouter.addLiquidityETH(
        tokenA.address,
    ...
    it("User completes zapIn with tokenA (pair tokenA/tokenC)", async function () {
      ...
      const estimation = await swapfishZap.estimateZapInSwap(tokenToZap, parseEther("1"), lpToken);
      ...
      const result = await swapfishZap.zapInToken(tokenToZap, tokenAmountIn, lpToken, minTokenAmountOut, {
        from: carol,
      });
      ...
      console.info("Balance tokenA: " + formatUnits(String(await tokenA.balanceOf(swapfishZap.address)), 18));
      console.info("Balance WBNB: " + formatUnits(String(await wrappedBNB.balanceOf(swapfishZap.address)), 18));
      console.info("Balance tokenC: " + formatUnits(String(await tokenC.balanceOf(swapfishZap.address)), 18));
    });
    ...
    it("User completes zapIn with BNB (pair BNB/tokenC)", async function () {
      ...
      const estimation = await swapfishZap.estimateZapInSwap(wrappedBNB.address, parseEther("1"), lpToken);
      assert.equal(estimation[2], tokenC.address);

      // Setting up slippage at 0.5%
      const minTokenAmountOut = new BN(estimation[1].toString()).mul(new BN("9995")).div(new BN("10000"));

      const result = await swapfishZap.zapInBNB(lpToken, minTokenAmountOut, {
        from: carol,
    ...
    it("User completes zapInRebalancing with BNB (pair BNB/tokenC)", async function () {
      ...
      const estimation = await swapfishZap.estimateZapInRebalancingSwap(
        wrappedBNB.address,
        tokenC.address,
        token0AmountIn,
        token1AmountIn,
      ...
      const result = await swapfishZap.zapInBNBRebalancing(
        tokenC.address,
        token1AmountIn,
        lpToken,
        maxTokenAmountIn,
    ...
    it("User completes zapInRebalancing with tokens (tokenA/tokenC)", async function () {
      ...
      const result = await swapfishZap.zapInTokenRebalancing(
        tokenA.address,
        tokenC.address,
        token0AmountIn,
        token1AmountIn,
    ...
    it("User completes zapOut to token (tokenA/tokenC)", async function () {
      ...
      const estimation = await swapfishZap.estimateZapOutSwap(lpToken, lpTokenAmount, tokenToReceive);
      assert.equal(estimation[2], tokenC.address);

      const minTokenAmountOut = new BN(estimation[1].toString()).mul(new BN("9995")).div(new BN("10000"));

      const result = await swapfishZap.zapOutToken(lpToken, tokenToReceive, lpTokenAmount, minTokenAmountOut, {
        from: carol,
      });
    ...
    it("User completes zapOut to BNB (BNB/tokenC)", async function () {
      ...
      const result = await swapfishZap.zapOutBNB(lpToken, lpTokenAmount, minTokenAmountOut, {
        from: carol,
      });
    ...
    it("Zap estimation fail if wrong tokens", async function () {
      await expectRevert(
        swapfishZap.estimateZapInSwap(wrappedBNB.address, parseEther("1"), pairAC.address),
        "Zap: Wrong tokens"
      );
      await expectRevert(
        swapfishZap.estimateZapInRebalancingSwap(
          tokenA.address,
          wrappedBNB.address,
          parseEther("1"),
          parseEther("1"),
      ...
      await expectRevert(
        swapfishZap.estimateZapOutSwap(pairAC.address, parseEther("1"), wrappedBNB.address),
        "Zap: Token not in LP"
      );
    });
    ...
    it("Zap estimations work as expected", async function () {
      ...
      const estimation0 = await swapfishZap.estimateZapInRebalancingSwap(
        tokenA.address,
        tokenC.address,
        parseEther("0.5"),
        parseEther("1"),
      ...
      const estimation1 = await swapfishZap.estimateZapInRebalancingSwap(
        tokenC.address,
        tokenA.address,
        parseEther("1"),
        parseEther("0.5"),
      ...
      const estimation2 = await swapfishZap.estimateZapInSwap(tokenA.address, parseEther("5"), pairAC.address);
      const estimation3 = await swapfishZap.estimateZapInRebalancingSwap(
        tokenA.address,
        tokenC.address,
        parseEther("5"),
        parseEther("0"),
    ...
    it("Cannot zap if wrong direction/tokens used", async function () {
      await expectRevert(
        swapfishZap.zapInToken(tokenA.address, parseEther("1"), pairBC.address, parseEther("0.51"), { from: carol }),
        "Zap: Wrong tokens"
      );
      await expectRevert(
        swapfishZap.zapInBNB(pairAC.address, parseEther("0.51"), { from: carol, value: parseEther("0.51").toString() }),
        "Zap: Wrong tokens"
      );

      await expectRevert(
        swapfishZap.zapOutToken(pairBC.address, tokenA.address, parseEther("0.51"), parseEther("0.51"), { from: carol }),
        "Zap: Token not in LP"
      );

      await expectRevert(
        swapfishZap.zapOutBNB(pairAC.address, parseEther("0.51"), parseEther("0.51"), { from: carol }),
        "Zap: Token not in LP"
      );

      await expectRevert(
        swapfishZap.zapInTokenRebalancing(
          tokenA.address,
          tokenC.address,
          parseEther("1"),
          parseEther("1"),
      ...
      await expectRevert(
        swapfishZap.zapInBNBRebalancing(
          tokenC.address,
          parseEther("1"),
          pairAB.address,
          parseEther("0.5"),
